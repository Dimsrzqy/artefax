<?php
// Services.php - FINAL ANIMATED VERSION
session_start();
...
// Query Produk
$q_best = "SELECT * FROM ((SELECT a.IDAlat AS id, 'alat' AS tipe, a.AlatNama AS name, a.AlatKategori AS kat, a.AlatDeskripsi AS des, a.AlatHarga AS prc, a.AlatDirGbr AS img, a.AlatStatus AS st FROM alat a WHERE AlatStatus!='Nonaktif') UNION ALL (SELECT p.IDPaket AS id, 'paket' AS tipe, p.PaketNama AS name, p.PaketKategori AS kat, p.PaketDeskripsi AS des, p.PaketHarga AS prc, p.PaketDirGbr AS img, p.PaketStatus AS st FROM paketjasa p WHERE PaketStatus!='Nonaktif')) AS gab ORDER BY (CASE WHEN LOWER(st)='bestseller' THEN 0 ELSE 1 END), id DESC LIMIT 8";
$res_best = $conn->query($q_best);

$q_new = "SELECT IDAlat AS id, 'alat' AS tipe, AlatNama AS name, AlatKategori AS kat, AlatDeskripsi AS des, AlatHarga AS prc, AlatDirGbr AS img, AlatStatus AS st FROM alat WHERE AlatStatus!='Nonaktif' UNION ALL SELECT IDPaket AS id, 'paket' AS tipe, PaketNama AS name, PaketKategori AS kat, PaketDeskripsi AS des, PaketHarga AS prc, PaketDirGbr AS img, PaketStatus AS st FROM paketjasa WHERE PaketStatus!='Nonaktif' ORDER BY id DESC LIMIT 8";
...
<body>
                    ...
                    <div class="search-box-hero position-relative mx-auto ms-lg-0 w-100" style="max-width: 600px;">
                        <form action="shop.php" method="GET" class="d-flex w-100">
                            <input class="form-control border-0 w-100 py-3 px-4 rounded-pill shadow-none bg-transparent" 
                                   type="text" 
                                   name="q" 
                                   placeholder="Cari kamera, lensa, atau paket..." 
                                   required>
    ...
    <?php function renderCard($p, $isFavorite = false) { 
        $img = (!empty($p['img']) && file_exists(__DIR__ . '/img/produk/' . $p['img'])) ? 'img/produk/' . $p['img'] : 'img/noimage.png';
        $status = $p['st'] ?? '';
    ...
    <div class="col-md-6 col-lg-4 col-xl-3">
        <div class="product-card">
            <div class="product-image-wrapper">
                <?php if ($isBest): ?>
                    <div class="position-absolute top-0 start-0 p-2 z-index-2"><span class="badge-fav"><i class="fa fa-star"></i> FAVORIT</span></div>
                <?php endif; ?>
            ...
            <div class="p-3 d-flex flex-column flex-grow-1">
                ...
                <div class="mt-auto pt-3 d-flex justify-content-between align-items-center border-top">
                    ...
                    <button class="btn-action openDetailBtn"
                        ...
                        data-price="<?= $p['prc'] ?>" data-desc="<?= htmlspecialchars($p['des']) ?>" data-img="<?= $img ?>"> 
    ...
    <div class="container py-5">
        ...
        <div class="row g-4">
            <?php while($row = $res_best->fetch_assoc()) renderCard($row, true); ?>
        </div>
    </div>
    <?php endif; ?>
    ...
    <div class="container py-5">
        ...
        <div class="row g-4">
            <?php while($row = $res_new->fetch_assoc()) renderCard($row); ?>
        </div>
        <div class="text-center mt-5">
            <a href="shop.php" class="btn btn-primary py-3 px-5 shadow-lg">Lihat Semua Produk <i class="fa fa-arrow-right ms-2"></i></a>
        </div>
    </div>
    ...
    <div class="modal fade" id="productModal" tabindex="-1"><div class="modal-dialog modal-lg modal-dialog-centered"><div class="modal-content border-0 shadow-lg"><div class="modal-header border-0 bg-light"><h5 class="modal-title fw-bold">Detail Produk</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div><div class="modal-body p-4"><div class="row g-4"><div class="col-md-6"><img id="modalImg" src="" class="img-fluid rounded shadow w-100" style="height:350px; object-fit:cover;"></div><div class="col-md-6"><h3 id="modalName" class="fw-bold mb-2"></h3><h4 class="fw-bold mb-4 text-warning" id="modalPrice"></h4><p id="modalDesc" class="text-muted mb-4"></p><div class="bg-light p-3 rounded border mb-4"><label class="fw-bold small mb-2 text-primary">Tanggal Sewa:</label><input type="date" id="modalDateInput" class="form-control fw-bold text-center border-primary" value="<?= date('Y-m-d') ?>"></div><input type="hidden" id="modalId"><input type="hidden" id="modalType"><div class="d-flex gap-3"><input type="number" id="modalQty" class="form-control w-25 text-center fw-bold" value="1" min="1"><button id="btnAddToCart" class="btn btn-primary w-100 fw-bold text-white shadow-sm"><i class="fa fa-cart-plus me-2"></i> Masuk Keranjang</button></div></div></div></div></div></div></div>
    ...
    $(document).ready(function() {
        $(document).on('click', '.openDetailBtn', function(e){
            ...
            $('#modalImg').attr('src', btn.data('img')); $('#modalName').text(btn.data('name')); $('#modalDesc').text(btn.data('desc')); $('#modalPrice').text(new Intl.NumberFormat('id-ID',{style:'currency',currency:'IDR',minimumFractionDigits:0}).format(btn.data('price'))); $('#modalId').val(btn.data('id')); $('#modalType').val(btn.data('tipe')); $('#modalDateInput').val(new Date().toISOString().split('T')[0]); new bootstrap.Modal(document.getElementById('productModal')).show();
        });
        $('#btnAddToCart').click(function(e){
            e.preventDefault(); const btn=$(this); const d=$('#modalDateInput').val(); if(!d){alert('Pilih tanggal!');return;} btn.prop('disabled',true).html('Loading...');
            $.ajax({ url:'root/cart_add.php', method:'POST', data:{id:$('#modalId').val(), type:$('#modalType').val(), qty:$('#modalQty').val(), date:d, name:$('#modalName').text(), price:$('#modalPrice').text().replace(/[^0-9]/g,'')}, dataType:'json', success:function(r){ if(r.status==='success'){ $('#jmlKeranjang').text(r.cart_count); btn.html('Berhasil!'); setTimeout(()=>{bootstrap.Modal.getInstance(document.getElementById('productModal')).hide();btn.prop('disabled',false).html('Masuk Keranjang');},800); }else{alert(r.message);btn.prop('disabled',false).html('Masuk Keranjang');} } });
        });
    });
    </script>
</body>
