...
<head>
  ...
  <style>
    ...
    .table thead th { background: #3366ff !important; color: #ffffff !important; font-weight: 600; text-transform: uppercase; font-size: 13px; letter-spacing: 0.5px; border: none; padding: 15px 20px; }
    .table tbody td { padding: 18px 20px; vertical-align: middle; border-top: 1px solid #eef2f7; font-size: 14px; }
    .table tbody tr:hover { background: #f1f5f9; transition: all 0.2s ease; }
  </style>
</head>
<body class="az-body">
          ...
          <?php if ($bookings): ?>
            <table class="table table-bordered table-hover">
              <thead class="thead-light">
                <tr>
                  <th>No</th><th>Customer</th><th>Paket</th><th>Tanggal</th><th>Alamat</th><th>Harga</th><th>Aksi</th>
                </tr>
              </thead>
              <tbody>
                  ...
                  <tr>
                    ...
                    <td>
                      <button class="btn btn-primary btn-sm"
                        onclick="openPopup(<?= $b['IDBooking'] ?>, '<?= addslashes(htmlspecialchars($b['NamaPaket'] ?? 'Event Jasa')) ?>', '<?= addslashes(htmlspecialchars($b['BkgAlamat'])) ?>', '<?= date('Y-m-d', strtotime($b['BkgTglMulai'])) ?>')">
                        Tugaskan
                      </button>
                    </td>
                  </tr>
    ...
    <div class="modal-dialog modal-lg">
      <div class="modal-content p-4 bg-white rounded shadow">
        ...
        <form id="formEvent" method="POST">
          ...
          <div class="row">
            <div class="col-md-12 mb-3"><label>Nama Event</label><input type="text" name="event_nama" id="event_nama" class="form-control" readonly></div>
          </div>
          <div class="row">
            <div class="col-md-12 mb-3"><label>Lokasi</label><input type="text" name="event_lokasi" id="event_lokasi" class="form-control" readonly></div>
          </div>
          <div class="row">
            <div class="col-md-4 mb-3"><label>Tanggal Event</label><input type="date" name="event_tanggal" id="event_tanggal" class="form-control" required></div>
            <div class="col-md-4 mb-3"><label>Jam Mulai</label><input type="time" name="event_mulai" id="event_mulai" class="form-control" required></div>
            <div class="col-md-4 mb-3"><label>Durasi (jam)</label><input type="number" name="event_durasi" id="event_durasi" class="form-control" min="1" max="24" value="8" required></div>
          </div>

          <div class="mb-3">
            <label>Pilih Karyawan <small class="text-muted">(bisa lebih dari satu)</small></label>
            <select name="karyawan[]" id="selectKaryawan" multiple class="form-control" style="height: 180px;" required></select>
            <small class="text-danger" id="infoBentrok" style="display:none;"></small>
          </div>
...
<script>
    ...
    function updateKaryawanList() {
        ...
        $.get('get_karyawan_tersedia.php', {
            ...
            rebuildKaryawanList(busyIds || []);
        }, 'json');
    }

    function rebuildKaryawanList(busyIds) {
        const $select = $('#selectKaryawan');
        $select.empty();

        let tersedia = 0;
        semuaKaryawan.forEach(k => {
            if (!busyIds.includes(k.id)) {
                $select.append(new Option(k.nama, k.id));
                tersedia++;
            }
        });

        if (tersedia === 0) {
            $select.append('<option value="">Tidak ada karyawan tersedia</option>');
