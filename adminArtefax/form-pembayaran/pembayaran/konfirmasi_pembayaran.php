...
<body class="az-body">
            ...
            <div class="az-content-left az-content-left-components d-lg-block d-none">
                <div class="component-item">
                    ...
                    <nav class="nav flex-column">
                        ...
                        <a href="../pembayaran/konfirmasi_pembayaran.php" class="nav-link active">Konfirmasi Pembayaran</a>
                    </nav>
            ...
            <div class="az-content-body pd-lg-l-40 d-flex flex-column">
                ...
                <div class="row">
                    <?php if ($pendingPayments && count($pendingPayments) > 0): ?>
                        <?php foreach ($pendingPayments as $index => $p): 
    ...
    <!-- POPUP DETAIL + LIGHTBOX (SEMUA DI DALAM 1 FILE) -->
    ...
    <script>
        ...
        function konfirmasiAksi(id, aksi) {
            ...
            if (aksi === 'setuju') {
                ...
                message.innerHTML = 'Setujui pembayaran ini?<br>Status menjadi <strong>Lunas</strong>.';
                btn.textContent = 'Ya, Setujui';
            } else {
                header.className = 'modal-header tolak';
                title.textContent = 'Tolak Pembayaran';
                message.innerHTML = 'Tolak pembayaran ini?<br>Status menjadi <strong>Gagal</strong>.';
                btn.textContent = 'Ya, Tolak';
            }
            btn.onclick = () => window.location.href = `proses_konfirmasi.php?id=${currentId}&aksi=${aksi}`;
        ...
        function openDetailPopup(data) {
            ...
            // BUKTI PEMBAYARAN - PATH BENAR
            let buktiHtml = '<em style="color:#888">Tidak ada bukti pembayaran</em>';
            if (data.PbrBukti && data.PbrBukti.trim() !== '') {
                const cleanFile = data.PbrBukti.trim().replace(/^(\.+[\/\\])+/g, '');
                const imgSrc = '../../../uploads/bukti_pembayaran/' + cleanFile;   // PATH PASTI BENAR

                buktiHtml = `
                    <div class="bukti-container">
                        <img src="${imgSrc}" class="bukti-thumbnail" alt="Bukti Transfer"
                        ...
                        <button class="btn-bukti" onclick="openLightbox('${imgSrc}')">
                            <i class="fas fa-expand-arrows-alt"></i> Lihat Ukuran Penuh
                        </button>
                    </div>`;
            }
            ...
            const html = `
                <div class="section">
                    ...
                    <table class="info-table">
                        ...
                        <tr><td>Bukti Transfer</td><td>${buktiHtml}</td></tr>
                    </table>
                </div>
                <div class="section">
                    <h3><i class="fas fa-user"></i> Pelanggan & Lokasi</h3>
                    <table class="info-table">
                ...
                <div class="section">
                    <h3><i class="fas fa-box"></i> Daftar Pesanan</h3>
                    <p><strong>Jenis Booking:</strong> ${data.JenisBooking || '-'}</p>
                    <ul class="item-list">${items}</ul>
                </div>
            `;

            document.getElementById('popupTitle').textContent = `Detail BOOK#${String(data.IDBooking||'').padStart(6,'0')}`;
            ...
            if (!lb.innerHTML) {
                lb.innerHTML = `
                    <div class="lightbox-overlay">
                        ...
                        <img class="lightbox-image" src="" alt="Bukti Pembayaran">
                        <div class="lightbox-caption">Klik di luar atau tekan ESC untuk menutup</div>
                    </div>`;
